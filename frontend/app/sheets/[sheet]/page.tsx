...
export default function SheetPage() {
  ...
  useEffect(() => {
    fetch('/api/sheets')
      .then(res => res.json())
      .then((data: { name: string }[]) => setSheets(data.map(d => d.name)));
  }, []);
  ...
  const addSheet = async () => {
    const res = await fetch('/api/sheets', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' });
    const data = await res.json();
    setSheets(prev => [...prev, data.name]);
    router.push(`/sheets/${data.name}`);
  };

  const deleteCurrentSheet = async () => {
    if (sheets.length <= 1) return;
    await fetch(`/api/sheets/${sheet}`, { method: 'DELETE' });
    const remaining = sheets.filter(s => s !== sheet);
    setSheets(remaining);
    router.push(`/sheets/${remaining[0]}`);
  };
    ...
    <main className={styles.container}>
      ...
      <div className={styles.sheetTabs}>
        {sheets.map((name) => (
          <button
            key={name}
            onClick={() => router.push(`/sheets/${name}`)}
            className={name === sheet ? styles.activeSheetTab : styles.sheetTab}
          >
            {name}
          </button>
        ))}
        <button onClick={addSheet} className={styles.sheetTab}>+</button>
        <button onClick={deleteCurrentSheet} className={styles.sheetTab}>-</button>
      </div>
    </main>
  );
}
